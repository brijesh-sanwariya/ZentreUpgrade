...
function Edit({
  ...
  const tel = useRef<string>(userInfo.Tel)
    ...
    const _onSave = async () => {
      ...
      let res = await dispatch(
        editContact({
          ...
          Email: email.current,
          Tel: tel.current,
          //CountryBorn: this.country ? this.country.id : userInfo.CountryBornID,
          Country: citizen.current,
          HeadPic: avatarPath
        })
      )
    ...
    <View
      ...
      <ScrollView
        ...
        <DropDownList
          ...
          title={t('citizenship')}
          items={nationalities}
          value={_getNationality()}
          onSelected={(n) => {
            //console.debug(n)
            citizen.current = n.id
          }}
        />
      </ScrollView>
    </View>
